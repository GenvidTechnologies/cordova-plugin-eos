...
- npm publishing uses the shared `genvid-holdings/genvid-public-ci` recipe: `.github/workflows/ci.yml` runs the node gate (`lint` → `typecheck` → `test` → `build`, plus a publish dry-run) on PRs and main; `.github/workflows/publish.yml` publishes `@genvid/cordova-plugin-eos` to npm with provenance via **OIDC trusted publishing** (no stored npm token) when a `vX.Y.Z` tag is pushed, after asserting the tag equals `package.json` version. To release: bump the version everywhere `version-guard` checks, then push a matching `vX.Y.Z` tag. `typecheck`/`test`/`build` exist only to satisfy the gate (this plugin ships sources as-is — `test`/`build` are honest no-ops; `typecheck` runs `tsc --noEmit` over `types/index.d.ts`). The previous CircleCI flow that uploaded `.tgz`s to Azure blob storage is **not** replicated.
